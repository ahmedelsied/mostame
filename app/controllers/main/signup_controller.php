...
class signup_controller extends controller
{
    ...
    protected $active_page = 'signup';
    private $full_name;
    private $email;
    private $pass;
    private $is_pass_confirmed;
    private $city;
    private $birthdate;
    private $gender;
    private $new_user;
    ...
    public function default_action()
    {
        ...
        $this->_view(MAIN_VIEWS.'signup');
        $this->loadFooter(MAIN_TEMP);
    }
    public function signup_process_action()
    {
        $this->post('signup_process_logic');
    }
    private function signup_process_logic($params)
    {
    ...
    public function signup_with_google_action()
    {
        $this->ajax("google_signup");
    }
    private function google_signup($params)
    {
        ...
        $this->new_user->status = ACTIVE_USER;
        $this->new_user->type = USER;
    ...
    public function signup_with_facebook_action()
    {
        $this->ajax("FB_signup");
    }
    private function FB_signup($params)
    {
        if(!$this->check_token($params)){
            $this->x_http_msg($this->__("backend_messages.messages.signup_API_error"),"danger");
        }
    ...
    private function log_user()
    {
        ...
        $this->set_session("permission",USER);
        $this->set_session("logged",USERS_ARRAY[USER]);
        $this->set_session("id",$id);
        $this->set_session("type",USER);
        $this->set_session("full_name",$this->new_user->full_name);
        $this->set_session("email",$this->new_user->email);
        $this->set_session("password",$this->new_user->password);
        $this->set_session("city",$this->new_user->city);
        $this->set_session("birthdate",$this->new_user->birthdate);
        $this->set_session("gender",$this->new_user->gender);
    }
}