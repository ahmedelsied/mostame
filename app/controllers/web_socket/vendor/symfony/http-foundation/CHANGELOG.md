 ...
 * made `Request::get_session()` throw if the session has not been set before
 ...
 * deprecated `Request::get_session()` when no session has been set. Use `Request::hasSession()` instead.
